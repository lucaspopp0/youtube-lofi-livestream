...
After that, you can add your MP3 audio files to the `resources/music_files` directory,
which will gradually be incorporated into the video.
You will see the progress percentage in the console.
The finalized videos will be saved in the `resources/ready_videos` directory.
...
PATH_TO_VIDEOS_FOLDER = os.path.join(ROOT_DIRECTORY, 'resources/ready_videos')
...
def run():
    while True:
        time.sleep(1)
        audios = get_file_paths_and_names(os.path.join(ROOT_DIRECTORY, 'resources/music_files'), '.mp3')
        ...
        for audio_name in render_needed:
            ...
            try:
                ...
                background_path = os.path.join(ROOT_DIRECTORY, 'resources/image.jpg')
